# generates a random data file with a specified size
...
def generate_random_buffer(size_in_bytes):
    random_buffer = bytes([random.randint(0, 255) for _ in range(size_in_bytes)])
    return random_buffer

def generate_random_file(file_path, file_size_in_bytes):
    random_data = generate_random_buffer (file_size_in_bytes)
